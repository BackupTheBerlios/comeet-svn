...
$queryEDetalle="SELECT SE.SGD_EXP_NUMERO, MINEXP.RADI_NUME_RADI, MINEXP.SGD_EXP_ESTADO, SE.SGD_SEXP_FECH, 
                     SE.SGD_EXP_FECH_ARCH, B.USUA_NOMB,EX.NUME_RADICADOS,SE.SGD_EXP_PRIVADO, R.RADI_FECH_RADI, 
                     R.RADI_PATH{$seguridad}
FROM SGD_SEXP_SECEXPEDIENTES SE INNER JOIN SGD_EXP_EXPEDIENTE EXP ON SE.SGD_EXP_NUMERO=EXP.SGD_EXP_NUMERO INNER JOIN ( SELECT RADI_NUME_RADI, SGD_EXP_ESTADO, MIN(SGD_EXP_FECH) 
FROM SGD_EXP_EXPEDIENTE WHERE DEPE_CODI=211 GROUP BY SGD_EXP_ESTADO , RADI_NUME_RADI) MINEXP ON EXP.radi_nume_radi=MINEXP.radi_nume_radi INNER JOIN RADICADO R
ON R.RADI_NUME_RADI=MINEXP.RADI_NUME_RADI JOIN USUARIO B ON B.USUA_DOC=SE.USUA_DOC INNER JOIN ( SELECT SGD_EXP_NUMERO, COUNT(*) NUME_RADICADOS 
FROM SGD_EXP_EXPEDIENTE GROUP BY SGD_EXP_NUMERO ) EX ON EX.SGD_EXP_NUMERO=SE.SGD_EXP_NUMERO 
            {$subcons}
            {$usuario} ";
...
{
	$titulos=$agrupamiento=="D.DEPE_CODI,D.DEPE_NOMB"?array("#","2#DEPENDENCIA","2#NUMERO DE EXPEDIENTES"):array("#","1#USUARIO","2#NUMERO DE EXPEDIENTES");
} 
