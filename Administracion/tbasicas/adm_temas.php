<?php
/*  Administrador de Tablas Tem�ticas.
 *	Cada tema puede pertenecer a uno o mas dependencias.
		...
		{	Case 'Agregar':
				{
					...
					foreach ($_POST['slc_idep'] as $dep)
					{
						$ok2 = $conn->Replace('SGD_TMD_TEMADEPE',
												array(  'ID'=>$conn->GenId('SEC_TMD_TEMADEPE'),
...
<tr bordercolor="#FFFFFF"> 
	<td align="left" class="titulos2">
		...
		<font color="White"> Para seleccionar varias dependencias ub&iacute;quese en el combo y mientras presiona la tecla &lt;CTRL&gt;, seleccione con la barra espaciadora o clic del rat&oacute;n</font>
	</td>
...
{		
	...
	if (accion =='Eliminar')
	{
		...
		{
			a = window.confirm('Esta seguro de eliminar el registro ?');
			if (a==true)
			{
				return true;
			}
