...

{
  ...
  * Constructor de la clase Historico
  ...
  function insertarHistorico($radicados,  $depeOrigen , $usCodOrigen, $depeDestino,$usCodDestino, $observacion, $tipoTx)
  {
		...
		//Modificado SKINA-IDRD -comillas doble-
		...
		{	# Asignar el valor de los campos en el registro
			...
			//Modificacion skina al machete
			if($usDocOrigen=='' || $usDocDestino=='') 
			{
			$usDocDestino = $rs->fields["usua_doc"];
			$usDocOrigen = $rs->fields["usua_doc"];
			}
						if(strlen($noRadicado)==14)
			{
				$nuRadicado=$noRadicado;
			}
			else{
			$trozos = explode("-", $noRadicado);
				if(strlen($trozos[0])==14)
					$nuRadicado=$trozos[0];
				if(strlen($trozos[1])==14)
				$nuRadicado=$trozos[1];
			}
			$record["RADI_NUME_RADI"] = $nuRadicado;
