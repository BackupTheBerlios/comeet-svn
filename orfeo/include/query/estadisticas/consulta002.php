...
{	//case 'oracle':
	...
	default:
		{	// Este default trabaja con Mssql 2K, 2K5 y Postgres. Para otras implementaciones tener en cuenta IF NULL. Linea 94
			if ( $dependencia_busq != 99999)
			{	$condicionE = "	AND $tmp_substr($radi_nume_radi,5,3)=$dependencia_busq AND b.depe_codi = $dependencia_busq";	}
			$queryE = "SELECT c.mrec_desc AS MEDIO_RECEPCION, 
						COUNT(*) AS Radicados, max(c.MREC_CODI) AS HID_MREC_CODI
					FROM RADICADO r
						LEFT JOIN MEDIO_RECEPCION c ON r.mrec_codi=c.mrec_codi
						LEFT JOIN USUARIO b ON r.radi_usua_radi=b.usua_CODI
					WHERE ".
			...
			$mrecCodi = ($mrecCodi == '') ? "c.mrec_codi IS NULL" : "c.mrec_codi = ".$mrecCodi;
			$queryEDetalle = "SELECT $radi_nume_radi RADICADO, 
						".$db->conn->SQLDate('Y/m/d h:i:s','r.radi_fech_radi')." FECHA_RADICADO
						,c.MREC_DESC MEDIO_RECEPCION
						,r.RA_ASUN ASUNTO
						,b.usua_nomb USUARIO
						,r.RADI_PATH HID_RADI_PATH{$seguridad}
