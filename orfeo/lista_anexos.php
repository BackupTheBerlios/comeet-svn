...
//Modificado SKINA-IDRD para postgres -as- 
$isql = "select  AN.anex_codigo as DOCU
            	,AT.anex_tipo_ext as EXT
	    	,AN.anex_tamano as TAMA
	    	,AN.anex_solo_lect as RO
            	,usua_nomb as CREA
		...
		,ANEX_TIPO as NUMEXTDOC
		 from anexos AN, anexos_tipo AT,usuario U
...
function ver_tipodocuATRD(anexo,codserie,tsub)
{
  <?php
		...
		$ind_ProcAnex="S";
  ?>
  window.open("./radicacion/tipificar_documento.php?krd=<?=$krd?>&nurad="+anexo+"&ind_ProcAnex=<?=$ind_ProcAnex?>&codusua=<?=$codusua?>&coddepe=<?=$coddepe?>&tsub="+tsub+"&codserie="+codserie,"Tipificacion_Documento_Anexos","height=500,width=750,scrollbars=yes");
}
  ...
  <?
	...
	<?php
	  ...
	  if(($rs->fields["EXT"]=="rtf" or $rs->fields["EXT"]=="doc" or $rs->fields["EXT"]=="odt" or $rs->fields["EXT"]=="xml") AND $rs->fields["ANEX_ESTADO"]<=3)
	   {
//             $linkarchivo .= "?time=".time();
  //           $linkarchivotmp .= "?time=".time();
