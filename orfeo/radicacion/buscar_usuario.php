...
{	if (document.formu1.idcont4.value && document.formu1.idpais4.value && document.formu1.codep_us4.value && document.formu1.muni_us4.value == 0)
	{	alert("Seleccione la geograf�a completa del destinatario");
		return false;
	}
	if (document.formu1.direccion_nus1.value=='')
	{	alert("Debe colocar una Direcci�n.");
		return false;
	}
...
<tr class="grisCCCCCC" align="center"> 
	...
	<? if($tbusqueda==1)
		{
	?>
		<td width="5%" CLASS="titulos5" >En RUPS</td>
	<? 
		}
  ...
  <?
   ...
   if($agregar=="AGREGAR" and $tagregar==2)
   {
		...
		if ($nextval==-1)
		{	die ("<span class='etextomenu'>No se encontrï¿½ la secuencia sec_oem_oempresas ");						   
		}
		...
		$isql = "INSERT INTO SGD_OEM_OEMPRESAS( tdid_codi, SGD_OEM_CODIGO, SGD_OEM_NIT, SGD_OEM_OEMPRESA, SGD_OEM_DIRECCION, 
				...
				values (4, $nextval, '$no_documento', '$nombre_nus1', '$direccion_nus1', '$seg_apell_nus1', '$telefono_nus1',
						$idcont4, $idpais4, $dpto_tmp, $muni_tmp)";
		$rs=$db->query($isql);
...
{	if($tbusqueda==0)
	...
	{	$isql = "select NIT_DE_LA_EMPRESA AS SGD_CIU_CEDULA,NOMBRE_DE_LA_EMPRESA as SGD_CIU_NOMBRE,SIGLA_DE_LA_EMPRESA as SGD_CIU_APELL1, ".
				...
				"CODIGO_DEL_MUNICIPIO as MUNI_CODI,ID_PAIS, ID_CONT, FLAG_RUPS from BODEGA_EMPRESAS ".
				"WHERE (SIGLA_DE_LA_EMPRESA LIKE '%$nombre_essp%' OR NOMBRE_DE_LA_EMPRESA LIKE '%$nombre_essp%') ";
...
?>
				<tr class='<?=$grilla ?>'> 
					...
					<? if($tbusqueda==1)
						{
							$enRUPS = $rs->fields["FLAG_RUPS"];
					?>
							<TD class="listado5"> <font size="-3"> <?=($enRUPS == 'S') ? '<center>SI</center>' : '<center>NO</center>'  ?></font></TD>
					<? 
						}
					?>
					<TD width="6%" align="center" valign="top" class="listado5">
						<font size="-3"><a href="#" onClick="pasar('<?=$i ?>',1);" class="titulos5"><?=$label_us?></a></font>
					</TD>
    <? 
	...
	?>
					<td width="6%" align="center" valign="top" class="listado5">
						<font size="-3"><a href="#" onClick="pasar('<?=$i ?>',2);" class="titulos5"><?=$label_pred?></a></font>
					</td>
    <?
					if($tbusqueda==1)
					{
	?>
						<td width="7%" align="center" valign="top" class="listado5"><font size="-3">
							<a href="#" onClick="pasar('<?=$i ?>',3);" class="titulos5"><?=$label_emp?></a></font>
						</td>
    <?
					}
				}
...
<td align="center" class="listado5"><font face="Arial, Helvetica, sans-serif"><?=$nombreTp3?></font></td>
  ...
  <?
	...
	{
	?>
      <input type=submit name=grb_destino value='Grabar el Destino de Este <?=$_SESSION["descRadicado"]?>' class="botones:largo">
      <input type=hidden name=verrad_sal value='<?=$verrad_sal?>' >
    <? 
	}else
	{
	?>
       <B><A href="javascript:pasar_datos('<?=$fechah?>');" ><span class="botones_largo">PASAR DATOS AL FORMULARIO</span></a></B>
      <input type=hidden name=verrad_sal value='<?=$verrad_sal?>' >
    <?
	}
	?> 
...
<tr align="center" > 
	...
	<td CLASS=titulos5><?=$_SESSION["descDepartamento"]?> / Estado</font></td>
	<td CLASS=titulos5><?=$_SESSION["descMunicipio"]?></font></td>
