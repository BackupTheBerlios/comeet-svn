...
$pathIni = "../bodega/tmp/Estadistica_". $tipoEstadistica. "_".date("Ymd_hms")."-1.csv";
$pathFin = "../bodega/tmp/Estadistica_". $tipoEstadistica. "_".date("Ymd_hms").".csv";
$fp = fopen($pathIni, "w");
if ($fp)
{
	$rscon = $db->conn->Execute($queryE);
  	rs2csvfile($rscon, $fp);
}
fclose($fp);
...
$encabezado = array();
$datos = array();
//$interfaz->_AddLog('	Abre Archivo: '. $fname);
while ( ($data = fgetcsv($fp, 10000, ",") ) !== FALSE )
{
	...
	if ($j==0)
	{
		for ($c=0; $c < $num; $c++)
		{
			if (substr($data[$c], 0, 3) != 'HID')
			{
				$posicion[$c] = $c;
				$encabezado[$c] = $data[$c];
			}
		}
		fputcsv($fpf, $encabezado, ',');
	}
	else
	{
		for ($c=0; $c < $num; $c++)
		{
			for ($d=0; $d < count($posicion); $d++)
			{
				if ($posicion[$c] == $c)
					$datos[$c] = $data[$c];
			}
...
echo "&nbsp;&nbsp;<a href='$pathFin'  class='vinculos' target='".date("dmYh").time("his")."'>Abrir Archivo CSV</a>";
*/
