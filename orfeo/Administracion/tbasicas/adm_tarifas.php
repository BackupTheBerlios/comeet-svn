	...
	{	$record = array();
		...
		{	Case 'Agregar':
			...
			Case 'Eliminar':{	$record = array_slice($record, 0, 3);
								$conn->BeginTrans();
								//$conn->debug=true;
								$ok = $conn->Execute('DELETE FROM SGD_CLTA_CLSTARIF WHERE SGD_FENV_CODIGO='.$record['SGD_FENV_CODIGO'].' AND SGD_TAR_CODIGO='.$record['SGD_TAR_CODIGO'].' AND SGD_CLTA_CODSER='.$record['SGD_CLTA_CODSER']);
								if ($ok) $ok = $conn->Execute('DELETE FROM SGD_TAR_TARIFAS WHERE SGD_FENV_CODIGO='.$record['SGD_FENV_CODIGO'].' AND SGD_TAR_CODIGO='.$record['SGD_TAR_CODIGO'].' AND SGD_CLTA_CODSER='.$record['SGD_CLTA_CODSER']);
								if ($ok)
								{	$conn->CommitTrans();
									$error = 4;
								}
...
?>
	...
	<tr bordercolor="#FFFFFF">
	    ...
	    <td width="16%" align="center" class="titulos2"> Limite Inferior</td>
	...
	<tr bordercolor="#FFFFFF">
		<td colspan="2" class="listado2">
			<center><input name="txt_v1" id="txt_v1" type="text" size="5" maxlength="5"></center>
		</td>
		<td colspan="2" class="listado2">
			<center><input name="txt_v2" id="txt_v2" type="text" size="5" maxlength="5"></center>
		</td>
	</tr>
