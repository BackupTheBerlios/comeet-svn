...
{	$db->conn->SetFetchMode(ADODB_FETCH_ASSOC);
		...
		{	case 'Agregar':
			{
				...
				$ok2 = mkdir(str_replace('/',$tmp_obj->barra,$ruta_raiz).$tmp_obj->barra."bodega".$tmp_obj->barra.date('Y').$tmp_obj->barra.$record['DEPE_CODI'].$tmp_obj->barra."docs",0777,true);
			...
			{	/* Las reglas del negocio para la inactivacion de una dependencia son:
				   ...
				   c. La dependencia seleccionada deberá tener su consecutivo del respectivo
				      ...
				      V A L I D A C I O N     c     N O    R E A L I Z A D A
				*/
				...
				while ((list(, $tmp) = each ($Vec_Trad)) && $okc)
				{
					...
					if ($_POST['slc_tr'.$tmp1] != $record_ori['depe_rad_tp'.$tmp1])
					{
						...
						$vlr_nxt = $db->conn->GetID('SECR_TP'.$tmp1.'_'.$_POST['slc_tr'.$tmp1]);
						$vlr_act = $db->conn->GetID('SECR_TP'.$tmp1.'_'.$record_ori['depe_rad_tp'.$tmp1]);
						if ($vlr_nxt < $vlr_act)
						{
							$okc = false;
						}
			...
			case 'Eliminar':
			{	/*
				a. No debe tener histórico la actual dependencia(Consecuencia del anterior punto b).
				*/
	...
	$rs = $db->conn->CacheExecute(1,$sql1.$sql3);	//utilizamos este recorset para los combos de las dependencias y para traer los datos generales de todas las dependencias.
	if ($rs)
	{
		$id = '';
		$muni_us1 = "0";
...
if ($error)
{
	...
	{	case 1: // No conexion a BD
		...
		case 10: // Error en la eliminacion de la dependencia
			$error_msg .= "La dependencia NO puede ser eliminada.";
			break;
...
{	if (!(isPositiveInteger(document.formSeleccion.txtIdDep.value,false)))
	...
	if (accion =='Eliminar')
	{
		a = window.confirm('Est� seguro de eliminar el registro ?');
		if (a==true)
		{}
		else
		{
...
<?php
		echo $rs->GetMenu2('id',$txtIdDep,':&lt;&lt seleccione &gt;&gt;',false,false,'Class="select" Onchange="ver_datos(this.value)"');
		$rs->Move(0);
?>
	</td>
</tr>
...
<div class="tabber" id="tab1">
	...
	<tr>
		...
		<td colspan="5" class="listado2">
	<?php
			echo $rs->GetMenu2('Slc_dpadre',$Slc_dpadre,':&lt;&lt seleccione &gt;&gt;',false,false,'Class="select" id="Slc_dpadre"');
			$rs->Move(0);
	?>
		</td>
	</tr>
	<tr>
		...
		<td colspan="5" class="listado2">
	<?php
			echo $rs->GetMenu2('Slc_dterr',$Slc_dterr,':&lt;&lt seleccione &gt;&gt;',false,false,'Class="select" id="Slc_dterr"');
			$rs->Move(0);
	?>
		</td>
	</tr>
	<tr>
		...
		<td colspan="5" class="listado2">
	<?php
			echo $rs->GetMenu2('Slc_dvis[]',$Slc_dvis,false,true,10,'Class="select" id="Slc_dvis"');
			$rs->Move(0);
	?>
		</td>
	</tr>
