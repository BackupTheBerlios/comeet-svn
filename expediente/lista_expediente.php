...
<?
	...
	if($funExpediente == "INSERT_EXP") {
		$resultadoExp = $expediente->insertar_expediente($num_expediente,
								...
								$usua_doc);
		if($resultadoExp==1) {
			echo '<hr>Se anex�este radicado al expediente correctamente.<hr>';
		}else {
			echo '<hr><font color=red>No se anex�este radicado al expediente. V
...
if( $expedienteSeleccionado )
{
	...
	$isql = "select ";
	    ...
	    $isql .= " b.*,c.sgd_tpr_descrip, " . $fecha . "as FECHA_RAD ,
			a.RADI_CUENTAI, a.RA_ASUN ,a.RADI_PATH ,$radi_nume_radi as RADI_NUME_RADI,a.SGD_SPUB_CODIGO,
			a.RADI_USUA_ACTU,a.radi_depe_actu
		from
		sgd_exp_expediente b, radicado a, SGD_TPR_TPDCUMENTO c
		where
		b.sgd_exp_numero='$expedienteSeleccionado'
		and b.radi_nume_radi=a.radi_nume_radi
		and a.tdoc_codi=c.sgd_tpr_codigo
        AND b.SGD_EXP_ESTADO <> 2
 ...
 //echo "$isql";
  ...
  <TD align="center">
    ...
    <TD align="center">
      <a href="#" onClick="javascript:ordenarPor( 'b.SGD_EXP_SUBEXPEDIENTE' );">
      Subexpediente
      </a>
	</TD>
	</tr>
